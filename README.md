...
- 한국어 번역문과 원문을 함께 표시
- 저자/문헌, 생애, 출처, 작성 시기, 원문 언어 메타데이터 제공
- 한국어/원문 음성 읽기
- 읽는 구간 하이라이트
- 명언 복사
- KST 시계와 CRT 스캔라인/글로우/효과음
- 데스크톱과 모바일 반응형 레이아웃

## 데이터

명언 데이터는 `quotes-data.js`의 `window.QUOTE_DATA`에 정적으로 포함되어 있습니다.
...
또는 파일 탐색기에서 `C:\project\index.html`을 더블클릭해도 됩니다.
...
새 명언은 `quotes-data.js`의 `quotes` 배열에 추가합니다. 새 저자나 문헌이 필요하면 먼저 `authors`에 정보를 추가한 뒤 `authorId`로 연결합니다.
