...
첫 화면에서는 터미널 명령어와 시작 안내문만 표시하고, `실행`을 누르면 선택한 카테고리에 맞는 명언을 무작위로 출력합니다. 한글 번역 명언을 크게 보여주고, 원문과 저자/문헌/출처/언어/작성 시기 정보를 함께 표시합니다.
...
- 한글 읽기 / 원문 읽기
  ...
  - 명언 출력 후에는 명언, 저자/문헌, 출처를 순서대로 읽기
...
새 명언을 추가할 때는 기존 저자나 문헌이 있으면 `quotes`에만 추가하고, 새 저자나 문헌이라면 `authors`에 정보를 먼저 추가합니다.
...
- 출처 신뢰도 필드: 원전 인용, 문헌 인용, 귀속 인용 구분
